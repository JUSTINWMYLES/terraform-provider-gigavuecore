...
func (r *PcapProfileResource) readRemote(ctx context.Context, state *PcapProfileResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	if v, ok := data["pcapAll"]; ok {
		if arr, ok := v.([]any); ok {
			...
			if state.Id.IsNull() {
				resp.Diagnostics.AddWarning("Error reading gigavuecore_pcap_profile", "The identifier attribute \"id\" is null in state, so the matching collection element cannot be identified. Falling back to the first element; this may track the wrong instance when the collection has more than one.")
			} else {
