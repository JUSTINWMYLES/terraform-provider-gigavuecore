...
func (r *UpgradeClusterNodesImageAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "upgrade cluster nodes image", Attributes: map[string]schema.Attribute{"async": schema.BoolAttribute{MarkdownDescription: "if provided, the call returns immediately with the [202 Accepted] HTTP status code, and the image upgrade will complete in the background", Optional: true}, "cluster_ids": schema.ListAttribute{MarkdownDescription: "ids of clusters to upgrade image for. Every node in these clusters is upgraded", Required: true, ElementType: types.StringType}, "image_file_specs": schema.ListNestedAttribute{MarkdownDescription: "lists image files to use", Required: true, NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{"device_model": schema.StringAttribute{MarkdownDescription: "Gigamon physical device models", Required: true}, "file_path": schema.StringAttribute{MarkdownDescription: "image file path on the image server", Required: true}, "file_type": schema.StringAttribute{MarkdownDescription: "image file type. 'web' and 'gigasmart' are only applicable for G-series devices", Optional: true}, "target_slot": schema.StringAttribute{MarkdownDescription: "only applicable for 'gigasmart' images for 2404 G-series devices. Indicates the target GS card to upgrade", Optional: true}, "update_uboot": schema.BoolAttribute{MarkdownDescription: "only applicable for H-series nodes. indicates whether the uboot is to be upgraded as part of the image upgrade", Optional: true}}}}, "image_server": schema.StringAttribute{MarkdownDescription: "alias of an image file server. has to reference one of the existing image file server profiles", Required: true}, "node_ids": schema.ListAttribute{MarkdownDescription: "ids of individual nodes to upgrade image for", Optional: true, ElementType: types.StringType}, "reboot": schema.BoolAttribute{MarkdownDescription: "indicates whether nodes should reboot after image upgrade", Required: true}, "skip_not_reachable_devices": schema.BoolAttribute{MarkdownDescription: "indicates whether not-reachable nodes(if any) should be skipped and continue with image upgrade", Optional: true}}}
}
