...
func (d *GetAllGpfcpProfileDataSource) readListRemote(ctx context.Context, config *GetAllGpfcpProfileDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_all_gpfcp_profile", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_all_gpfcp_profile", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["gpfcpProfiles"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_all_gpfcp_profile", fmt.Sprintf("Could not decode list page: missing %q array", "gpfcpProfiles"))
			return
		}
