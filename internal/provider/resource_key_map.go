...
var (
	...
	_ resource.ResourceWithIdentity    = (*KeyMapResource)(nil)
...
type KeyMapResourceModel struct {
	...
	Timeouts  *KeyMapTimeoutsModel `tfsdk:"timeouts"`
}

// KeyMapTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_key_map resource.
type KeyMapTimeoutsModel struct {
...
func (r *KeyMapResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	keyMapImportIDParts := strings.Split(req.ID, "/")
	if len(keyMapImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("alias"), keyMapImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), keyMapImportIDParts[1])...)
}
