...
func (r *UpdateAlertPoliciesAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Update of list of given alert-policies", Attributes: map[string]schema.Attribute{"context": schema.DynamicAttribute{MarkdownDescription: "Gigamon query result context", Optional: true}, "policies": schema.DynamicAttribute{MarkdownDescription: "List of alert policies to be updated", Required: true}}}
}
