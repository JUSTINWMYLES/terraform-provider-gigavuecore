...
var _ list.ListResourceWithConfigure = (*GetAllGigaFlexInlineNetworkGroupListResource)(nil)
...
// GetAllGigaFlexInlineNetworkGroupListResourceModel describes the gigavuecore_get_all_giga_flex_inline_network_group list filter configuration shape.
type GetAllGigaFlexInlineNetworkGroupListResourceModel struct {
...
func (l *GetAllGigaFlexInlineNetworkGroupListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{MarkdownDescription: "Get All FM Inline Network Groups", Attributes: map[string]listschema.Attribute{"cluster_id": listschema.StringAttribute{MarkdownDescription: "if provided, Network groups only for that cluster is returned", Optional: true}}}
}

// List streams matching resource instances for terraform query.
func (l *GetAllGigaFlexInlineNetworkGroupListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config GetAllGigaFlexInlineNetworkGroupListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *GetAllGigaFlexInlineNetworkGroupListResource) listRemote(ctx context.Context, config *GetAllGigaFlexInlineNetworkGroupListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["gigaFlexInlineNetworkGroups"]
		if !ok {
			diags.AddError("Error listing gigavuecore_get_all_giga_flex_inline_network_group", fmt.Sprintf("Could not decode list page: missing %q array", "gigaFlexInlineNetworkGroups"))
			return nil, diags
		}
...
func (l *GetAllGigaFlexInlineNetworkGroupListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
