...
var _ list.ListResource = (*GroupListResource)(nil)
var _ list.ListResourceWithConfigure = (*GroupListResource)(nil)

// GroupListResource is the generated Terraform list resource implementation.
type GroupListResource struct {
	client *client.Client
}

// GroupListResourceModel describes the gigavuecore_group list filter configuration shape.
type GroupListResourceModel struct {
	Page types.String `tfsdk:"page"`
	Sort types.String `tfsdk:"sort"`
}

// NewGroupListResource returns a new instance of the generated list resource.
func NewGroupListResource() list.ListResource {
	return &GroupListResource{}
}

// Metadata returns the list resource type name.
func (l *GroupListResource) Metadata(_ context.Context, _ resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (l *GroupListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config GroupListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_group", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_group", "List item is missing identity attribute \"name\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_group", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_group", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_group", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *GroupListResource) listRemote(ctx context.Context, config *GroupListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_group", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_group", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["groups"]
		if !ok {
			diags.AddError("Error listing gigavuecore_group", fmt.Sprintf("Could not decode list page: missing %q array", "groups"))
			return nil, diags
		}
