...
var (
	_ datasource.DataSource              = (*GetClusterConfigImageUpgradeStatusByTaskIdDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*GetClusterConfigImageUpgradeStatusByTaskIdDataSource)(nil)
)

// GetClusterConfigImageUpgradeStatusByTaskIdDataSource is the generated Terraform data source implementation.
type GetClusterConfigImageUpgradeStatusByTaskIdDataSource struct {
	client *client.Client
}

// GetClusterConfigImageUpgradeStatusByTaskIdDataSourceModel describes the data source state shape.
type GetClusterConfigImageUpgradeStatusByTaskIdDataSourceModel struct {
...
func (d *GetClusterConfigImageUpgradeStatusByTaskIdDataSource) readRemote(ctx context.Context, config *GetClusterConfigImageUpgradeStatusByTaskIdDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
		resp.Diagnostics.AddError("Client Not Configured", "The API client was not set on the resource. The provider Configure method must run before resource operations; this is a bug in the generated provider.")
		return
	}
	reqPath := "/clusterConfig/imageUpgrade/status/{taskId}"
	reqPath = strings.ReplaceAll(reqPath, "{taskId}", url.PathEscape(config.TaskId.ValueString()))
	httpReq, err := d.client.NewRequest(ctx, http.MethodGet, reqPath, nil)
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", fmt.Sprintf("Could not build request: %s", err))
		return
	}
	httpResp, err := d.client.Do(httpReq)
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", fmt.Sprintf("Could not send request: %s", err))
		return
	}
	defer httpResp.Body.Close()
	if httpResp.StatusCode == http.StatusNotFound {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "The requested resource was not found.")
		return
	}
	if !(httpResp.StatusCode == 200) {
		switch httpResp.StatusCode {
		case 400:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Invalid request. See errors payload for details")
			return
		case 401:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Not Authenticated. See errors payload for details")
			return
		case 403:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Access Denied. See errors payload for details")
			return
		case 404:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Entity Not Found. See errors payload for details")
			return
		case 409:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Entity Already Exists. See errors payload for details")
			return
		case 500:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Internal Server Error. See errors payload for details")
			return
		case 503:
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", "Service Unavailable. See errors payload for details")
			return
		default:
			apiErr, err := client.NewAPIError(httpResp)
			if err != nil {
				resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", fmt.Sprintf("Could not read error response: %s", err))
				return
			}
			resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", apiErr.Error())
			return
		}
	}
	...
	if err := decoder.Decode(&data); err != nil && err != io.EOF {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", fmt.Sprintf("Could not decode response body: %s", err))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_cluster_config_image_upgrade_status_by_task_id", fmt.Sprintf("Could not map response to state: %s", err))
		return
	}
}

// Configure stores the API client supplied by the provider.
func (d *GetClusterConfigImageUpgradeStatusByTaskIdDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
