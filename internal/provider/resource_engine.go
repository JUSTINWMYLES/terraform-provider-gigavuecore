...
type EngineResourceModel struct {
	...
	Timeouts           *EngineTimeoutsModel `tfsdk:"timeouts"`
}

// EngineTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_engine resource.
type EngineTimeoutsModel struct {
...
func (r *EngineResource) readRemote(ctx context.Context, state *EngineResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	reqPath = strings.ReplaceAll(reqPath, "{eport}", url.PathEscape(strings.ReplaceAll(state.Eport.ValueString(), "/", "_")))
