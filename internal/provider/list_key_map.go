...
var _ list.ListResource = (*KeyMapListResource)(nil)
var _ list.ListResourceWithConfigure = (*KeyMapListResource)(nil)

// KeyMapListResource is the generated Terraform list resource implementation.
type KeyMapListResource struct {
	client *client.Client
}

// KeyMapListResourceModel describes the gigavuecore_key_map list filter configuration shape.
type KeyMapListResourceModel struct {
	ClusterId types.String `tfsdk:"cluster_id"`
	Page      types.String `tfsdk:"page"`
	Sort      types.String `tfsdk:"sort"`
}

// NewKeyMapListResource returns a new instance of the generated list resource.
func NewKeyMapListResource() list.ListResource {
	return &KeyMapListResource{}
}

// Metadata returns the list resource type name.
func (l *KeyMapListResource) Metadata(_ context.Context, _ resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "gigavuecore_key_map"
}

// ListResourceConfigSchema returns the list resource config schema.
func (l *KeyMapListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
...
func (l *KeyMapListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config KeyMapListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_key_map", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_key_map", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_key_map", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_key_map", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_key_map", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *KeyMapListResource) listRemote(ctx context.Context, config *KeyMapListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_key_map", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_key_map", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["keyMaps"]
		if !ok {
			diags.AddError("Error listing gigavuecore_key_map", fmt.Sprintf("Could not decode list page: missing %q array", "keyMaps"))
			return nil, diags
		}
...
func (l *KeyMapListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
