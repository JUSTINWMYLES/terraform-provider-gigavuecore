...
type EmailRecipientResourceModel struct {
	...
	Timeouts       *EmailRecipientTimeoutsModel `tfsdk:"timeouts"`
}

// EmailRecipientTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_email_recipient resource.
type EmailRecipientTimeoutsModel struct {
...
func (r *EmailRecipientResource) readRemote(ctx context.Context, state *EmailRecipientResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	if v, ok := data["recipients"]; ok {
		if arr, ok := v.([]any); ok {
			...
			if state.EmailAddress.IsNull() {
				resp.Diagnostics.AddWarning("Error reading gigavuecore_email_recipient", "The identifier attribute \"email_address\" is null in state, so the matching collection element cannot be identified. Falling back to the first element; this may track the wrong instance when the collection has more than one.")
			} else {
				want := fmt.Sprint(state.EmailAddress.ValueString())
				for _, item := range arr {
					if m, ok := item.(map[string]any); ok {
						if idVal, ok := m["emailAddress"]; ok && fmt.Sprint(idVal) == want {
							match = m
							break
						}
					} else {
