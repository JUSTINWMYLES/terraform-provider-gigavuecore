...
var _ list.ListResourceWithConfigure = (*GetAllTunnelApplicationListResource)(nil)
...
// GetAllTunnelApplicationListResourceModel describes the gigavuecore_get_all_tunnel_application list filter configuration shape.
type GetAllTunnelApplicationListResourceModel struct {
...
func (l *GetAllTunnelApplicationListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config GetAllTunnelApplicationListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_tunnel_application", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *GetAllTunnelApplicationListResource) listRemote(ctx context.Context, config *GetAllTunnelApplicationListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["tunnelApps"]
		if !ok {
			diags.AddError("Error listing gigavuecore_get_all_tunnel_application", fmt.Sprintf("Could not decode list page: missing %q array", "tunnelApps"))
			return nil, diags
		}
...
func (l *GetAllTunnelApplicationListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
