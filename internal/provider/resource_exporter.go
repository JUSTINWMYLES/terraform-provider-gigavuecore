...
var (
	...
	_ resource.ResourceWithIdentity    = (*ExporterResource)(nil)
...
type ExporterResourceModel struct {
	...
	Timeouts          *ExporterTimeoutsModel `tfsdk:"timeouts"`
}

// ExporterTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_exporter resource.
type ExporterTimeoutsModel struct {
...
func (r *ExporterResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	exporterImportIDParts := strings.Split(req.ID, "/")
	if len(exporterImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("alias"), exporterImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), exporterImportIDParts[1])...)
}
