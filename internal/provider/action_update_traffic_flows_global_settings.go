...
func (r *UpdateTrafficFlowsGlobalSettingsAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Update global settings for Traffic Flows", Attributes: map[string]schema.Attribute{"body": schema.DynamicAttribute{MarkdownDescription: "Settings for traffic flows, including migration and fabric resource configuration.", Required: true}}}
}
