...
type UpgradeClusterGsCardImageActionModel struct {
	ClusterSpecs            types.List   `tfsdk:"cluster_specs" json:"clusterSpecs"`
...
func (r *UpgradeClusterGsCardImageAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "upgrade the GS cards for selected clusters, it is an async only", Attributes: map[string]schema.Attribute{"cluster_specs": schema.ListNestedAttribute{MarkdownDescription: "Cluster wise gs image spec", Required: true, NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{"cluster_id": schema.StringAttribute{MarkdownDescription: "clusterId", Required: true}, "image_file_spec": schema.ListNestedAttribute{MarkdownDescription: "list of upgrade spec for the cluster", Required: true, NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{"box_id": schema.StringAttribute{MarkdownDescription: "boxId of the gs card to be upgraded", Optional: true}, "file_path": schema.StringAttribute{MarkdownDescription: "path of the image file", Required: true}, "product_code": schema.StringAttribute{MarkdownDescription: "product code of gs card to be upgraded", Required: true}, "target_slots": schema.ListAttribute{MarkdownDescription: "slotIds of the box to be to upgraded", Optional: true, ElementType: types.StringType}}}}}}}, "image_server": schema.StringAttribute{MarkdownDescription: "alias of an image file server. it has to reference one of the existing image file server profiles", Required: true}, "skip_not_reachable_devices": schema.BoolAttribute{MarkdownDescription: "indicates whether the selected GigaSMART cards which belong to not-reachable nodes(if any) should be skipped and continue with image upgrade", Optional: true}, "task_name": schema.StringAttribute{MarkdownDescription: "user provided task name for the image upgrade", Required: true}}}
}
