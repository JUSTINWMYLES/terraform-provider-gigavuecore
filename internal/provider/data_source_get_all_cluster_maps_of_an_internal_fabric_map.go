...
func (d *GetAllClusterMapsOfAnInternalFabricMapDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "Get all cluster maps of an internal fabric map", Attributes: map[string]schema.Attribute{"alias": schema.StringAttribute{MarkdownDescription: "alias of the fabric map", Required: true}, "ifm_alias": schema.StringAttribute{MarkdownDescription: "alias of the internal fabric map", Required: true}, "items": schema.DynamicAttribute{Computed: true}}}
}
...
func (d *GetAllClusterMapsOfAnInternalFabricMapDataSource) readListRemote(ctx context.Context, config *GetAllClusterMapsOfAnInternalFabricMapDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_all_cluster_maps_of_an_internal_fabric_map", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_all_cluster_maps_of_an_internal_fabric_map", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["clusterMaps"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_all_cluster_maps_of_an_internal_fabric_map", fmt.Sprintf("Could not decode list page: missing %q array", "clusterMaps"))
			return
		}
