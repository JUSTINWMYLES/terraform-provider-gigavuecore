...
func (d *GetUpgradeJobsDataSource) readListRemote(ctx context.Context, config *GetUpgradeJobsDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if !config.TaskId.IsNull() {
		params.Set("taskId", config.TaskId.ValueString())
	}
	if !config.TaskName.IsNull() {
		params.Set("taskName", config.TaskName.ValueString())
	}
	...
	if !config.ScheduledTime.IsNull() {
		params.Set("scheduledTime", config.ScheduledTime.ValueString())
	}
	if !config.CreatedBy.IsNull() {
		params.Set("createdBy", config.CreatedBy.ValueString())
	}
	if !config.TimeLeft.IsNull() {
		params.Set("timeLeft", config.TimeLeft.ValueString())
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_upgrade_jobs", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_upgrade_jobs", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["deviceUpgradeTaskInfoRecord"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_upgrade_jobs", fmt.Sprintf("Could not decode list page: missing %q array", "deviceUpgradeTaskInfoRecord"))
			return
		}
