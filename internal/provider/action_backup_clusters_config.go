...
type BackupClustersConfigActionModel struct {
	ClustersConfigSpecs types.Dynamic `tfsdk:"clusters_config_specs" json:"clustersConfigSpecs"`
}
...
func (r *BackupClustersConfigAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Backup of multiple clusters can be requested", Attributes: map[string]schema.Attribute{"clusters_config_specs": schema.DynamicAttribute{MarkdownDescription: "list of Clusters config backup references", Required: true}}}
}
