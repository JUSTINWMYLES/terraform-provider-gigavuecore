...
type UserRoleResourceModel struct {
	...
	Timeouts    *UserRoleTimeoutsModel `tfsdk:"timeouts"`
}

// UserRoleTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_user_role resource.
type UserRoleTimeoutsModel struct {
	...
	if v, ok := data["userRoles"]; ok {
		if arr, ok := v.([]any); ok {
			...
			if state.Name.IsNull() {
				resp.Diagnostics.AddWarning("Error reading gigavuecore_user_role", "The identifier attribute \"name\" is null in state, so the matching collection element cannot be identified. Falling back to the first element; this may track the wrong instance when the collection has more than one.")
			} else {
				want := fmt.Sprint(state.Name.ValueString())
				for _, item := range arr {
					if m, ok := item.(map[string]any); ok {
						if idVal, ok := m["name"]; ok && fmt.Sprint(idVal) == want {
							match = m
							break
						}
					} else {
...
func (r *UserRoleResource) updateRemote(ctx context.Context, plan *UserRoleResourceModel, resp *resource.UpdateResponse) {
	...
	reqPath = strings.ReplaceAll(reqPath, "{roleName}", url.PathEscape(plan.Name.ValueString()))
...
func (r *UserRoleResource) deleteRemote(ctx context.Context, state *UserRoleResourceModel, resp *resource.DeleteResponse) {
	...
	reqPath := "/system/aaa/roles/{roleName}"
	reqPath = strings.ReplaceAll(reqPath, "{roleName}", url.PathEscape(state.Name.ValueString()))
...
func (r *UserRoleResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	userRoleImportIDParts := strings.Split(req.ID, ":")
	if len(userRoleImportIDParts) != 2 {
		resp.Diagnostics.AddError("Unexpected Import Identifier", fmt.Sprintf("Expected import identifier with format \"{name}:{cluster_id}\". Got %q.", req.ID))
		return
	}
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("name"), userRoleImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), userRoleImportIDParts[1])...)
}
