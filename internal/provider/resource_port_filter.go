...
func (r *PortFilterResource) Schema(ctx context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "get filter of a Port by port ID", Attributes: map[string]schema.Attribute{"cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true}, "port": schema.StringAttribute{Required: true}, "port_id": schema.StringAttribute{MarkdownDescription: "id of the target device Port (format: boxId_slotId_port, example: 1_1_c1)", Computed: true}, "rules": schema.SingleNestedAttribute{MarkdownDescription: "Port Filter Rules Container. Private class", Required: true, Attributes: map[string]schema.Attribute{"drop_rules": schema.DynamicAttribute{Optional: true, Computed: true}, "pass_rules": schema.DynamicAttribute{Optional: true, Computed: true}}}}, Blocks: map[string]schema.Block{"timeouts": timeouts.Block(ctx, timeouts.Opts{Create: true, Read: true, Update: true, Delete: true})}}
}
