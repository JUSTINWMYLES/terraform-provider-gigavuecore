...
import (
	...
	dynamicplanmodifier "github.com/hashicorp/terraform-plugin-framework/resource/schema/dynamicplanmodifier"
...
func (r *PortFilterResource) Schema(ctx context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "get filter of a Port by port ID", Attributes: map[string]schema.Attribute{"cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true, PlanModifiers: []planmodifier.String{stringplanmodifier.RequiresReplace()}}, "port": schema.StringAttribute{Required: true, PlanModifiers: []planmodifier.String{stringplanmodifier.RequiresReplace()}}, "rules": schema.SingleNestedAttribute{MarkdownDescription: "Port Filter Rules Container. Private class", Required: true, Attributes: map[string]schema.Attribute{"drop_rules": schema.DynamicAttribute{Optional: true, Computed: true, PlanModifiers: []planmodifier.Dynamic{dynamicplanmodifier.RequiresReplace()}}, "pass_rules": schema.DynamicAttribute{Optional: true, Computed: true, PlanModifiers: []planmodifier.Dynamic{dynamicplanmodifier.RequiresReplace()}}}, PlanModifiers: []planmodifier.Object{objectplanmodifier.RequiresReplace()}}}, Blocks: map[string]schema.Block{"timeouts": timeouts.Block(ctx, timeouts.Opts{Create: true, Read: true, Update: true, Delete: true})}}
}
...
func (r *PortFilterResource) readRemote(ctx context.Context, state *PortFilterResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	reqPath = strings.ReplaceAll(reqPath, "{portId}", url.PathEscape(state.Port.ValueString()))
