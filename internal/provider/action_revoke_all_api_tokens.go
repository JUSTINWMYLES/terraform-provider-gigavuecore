...
func (r *RevokeAllApiTokensAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Revoke token for FM Users", Attributes: map[string]schema.Attribute{"context": schema.DynamicAttribute{MarkdownDescription: "Gigamon query result context", Optional: true}, "fm_api_token_user_entities": schema.DynamicAttribute{Required: true}}}
}
