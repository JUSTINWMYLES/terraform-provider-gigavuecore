...
var _ list.ListResource = (*MonitorListResource)(nil)
var _ list.ListResourceWithConfigure = (*MonitorListResource)(nil)

// MonitorListResource is the generated Terraform list resource implementation.
type MonitorListResource struct {
	client *client.Client
}

// MonitorListResourceModel describes the gigavuecore_monitor list filter configuration shape.
type MonitorListResourceModel struct {
	ClusterId types.String `tfsdk:"cluster_id"`
	Page      types.String `tfsdk:"page"`
	Sort      types.String `tfsdk:"sort"`
}

// NewMonitorListResource returns a new instance of the generated list resource.
func NewMonitorListResource() list.ListResource {
	return &MonitorListResource{}
}

// Metadata returns the list resource type name.
func (l *MonitorListResource) Metadata(_ context.Context, _ resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (l *MonitorListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config MonitorListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_monitor", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_monitor", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_monitor", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_monitor", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_monitor", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *MonitorListResource) listRemote(ctx context.Context, config *MonitorListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_monitor", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_monitor", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["nfMonitors"]
		if !ok {
			diags.AddError("Error listing gigavuecore_monitor", fmt.Sprintf("Could not decode list page: missing %q array", "nfMonitors"))
			return nil, diags
		}
