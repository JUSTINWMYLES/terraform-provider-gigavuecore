...
var _ list.ListResourceWithConfigure = (*LoadFmBackupArchiveServersListResource)(nil)
...
func (l *LoadFmBackupArchiveServersListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", "List item is missing identity attribute \"server_alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *LoadFmBackupArchiveServersListResource) listRemote(ctx context.Context) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["fmBackupArchiveServers"]
		if !ok {
			diags.AddError("Error listing gigavuecore_load_fm_backup_archive_servers", fmt.Sprintf("Could not decode list page: missing %q array", "fmBackupArchiveServers"))
			return nil, diags
		}
...
func (l *LoadFmBackupArchiveServersListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
