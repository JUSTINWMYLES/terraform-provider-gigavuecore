...
var (
	...
	_ resource.ResourceWithIdentity    = (*TunnelLbEndpointResource)(nil)
...
type TunnelLbEndpointResourceModel struct {
	...
	Timeouts  *TunnelLbEndpointTimeoutsModel `tfsdk:"timeouts"`
}

// TunnelLbEndpointTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_tunnel_lb_endpoint resource.
type TunnelLbEndpointTimeoutsModel struct {
...
func (r *TunnelLbEndpointResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "Create a Tunnel Endpoint", Attributes: map[string]schema.Attribute{"alias": schema.StringAttribute{MarkdownDescription: "tunnel endpoint alias", Optional: true, Computed: true, Validators: []validator.String{stringvalidator.LengthAtLeast(1)}}, "cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true}, "ip_address": schema.StringAttribute{MarkdownDescription: "tunnel endpoint remote ip address. IPv4 or IPv6", Optional: true, Computed: true}, "te_id": schema.StringAttribute{MarkdownDescription: "tunnel endpoint alias, format: teN , where 1 <= N <= 128", Required: true, Validators: []validator.String{stringvalidator.LengthAtLeast(3)}}}, Blocks: map[string]schema.Block{"timeouts": schema.SingleNestedBlock{Attributes: map[string]schema.Attribute{"create": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the create operation. Defaults to 1200.", Optional: true}, "read": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the read operation. Defaults to 1200.", Optional: true}, "update": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the update operation. Defaults to 1200.", Optional: true}, "delete": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the delete operation. Defaults to 1200.", Optional: true}}, MarkdownDescription: "Per-operation timeouts in seconds."}}}
}

// IdentitySchema returns the resource identity schema shared with the paired list resource.
func (r *TunnelLbEndpointResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
	resp.IdentitySchema = identityschema.Schema{Attributes: map[string]identityschema.Attribute{"te_id": identityschema.StringAttribute{RequiredForImport: true, Description: "tunnel endpoint alias, format: teN , where 1 <= N <= 128"}}}
}
...
func (r *TunnelLbEndpointResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	resp.Diagnostics.Append(resp.Identity.SetAttribute(ctx, path.Root("te_id"), plan.TeId)...)
...
func (r *TunnelLbEndpointResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if r.readRemote(ctx, &state, resp) {
		resp.Diagnostics.Append(resp.Identity.SetAttribute(ctx, path.Root("te_id"), state.TeId)...)
...
func (r *TunnelLbEndpointResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	tunnelLbEndpointImportIDParts := strings.Split(req.ID, "/")
	if len(tunnelLbEndpointImportIDParts) != 2 {
		resp.Diagnostics.AddError("Unexpected Import Identifier", fmt.Sprintf("Expected import identifier with format \"{te_id}/{cluster_id}\". Got %q.", req.ID))
		return
	}
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("te_id"), tunnelLbEndpointImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), tunnelLbEndpointImportIDParts[1])...)
}
