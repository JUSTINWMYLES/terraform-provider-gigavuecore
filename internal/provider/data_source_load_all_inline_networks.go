...
func (d *LoadAllInlineNetworksDataSource) readListRemote(ctx context.Context, config *LoadAllInlineNetworksDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_load_all_inline_networks", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_inline_networks", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["inlineNetworks"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_inline_networks", fmt.Sprintf("Could not decode list page: missing %q array", "inlineNetworks"))
			return
		}
