...
var _ list.ListResourceWithConfigure = (*LoadAllGsopsListResource)(nil)
...
// LoadAllGsopsListResourceModel describes the gigavuecore_load_all_gsops list filter configuration shape.
type LoadAllGsopsListResourceModel struct {
...
func (l *LoadAllGsopsListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config LoadAllGsopsListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_gsops", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *LoadAllGsopsListResource) listRemote(ctx context.Context, config *LoadAllGsopsListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["gsops"]
		if !ok {
			diags.AddError("Error listing gigavuecore_load_all_gsops", fmt.Sprintf("Could not decode list page: missing %q array", "gsops"))
			return nil, diags
		}
...
func (l *LoadAllGsopsListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
