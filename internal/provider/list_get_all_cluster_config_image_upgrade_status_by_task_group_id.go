...
var _ list.ListResourceWithConfigure = (*GetAllClusterConfigImageUpgradeStatusByTaskGroupIdListResource)(nil)
...
func (l *GetAllClusterConfigImageUpgradeStatusByTaskGroupIdListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			taskGroupIdValue, ok := itemMap["taskGroupId"]
			if !ok {
				if itemMap["metadata"] != nil {
					metaMap := map[string]json.RawMessage{}
					if json.Unmarshal(itemMap["metadata"], &metaMap) == nil {
						taskGroupIdValue, ok = metaMap["taskGroupId"]
					}
				}
			}
			if !ok {
				taskGroupIdValue, ok = itemMap["task_group_id"]
			}
			if !ok {
				taskGroupIdValue, ok = itemMap["id"]
			}
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", "List item is missing identity attribute \"task_group_id\".")
				if !push(result) {
					return
				}
				continue
			}
			identity["task_group_id"] = taskGroupIdValue
			idJSON, err := json.Marshal(identity)
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *GetAllClusterConfigImageUpgradeStatusByTaskGroupIdListResource) listRemote(ctx context.Context) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["upgradeTaskStatus"]
		if !ok {
			diags.AddError("Error listing gigavuecore_get_all_cluster_config_image_upgrade_status_by_task_group_id", fmt.Sprintf("Could not decode list page: missing %q array", "upgradeTaskStatus"))
			return nil, diags
		}
...
func (l *GetAllClusterConfigImageUpgradeStatusByTaskGroupIdListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
