...
var _ list.ListResourceWithConfigure = (*LoadAllMetadataNetworkProfilesListResource)(nil)
...
// LoadAllMetadataNetworkProfilesListResourceModel describes the gigavuecore_load_all_metadata_network_profiles list filter configuration shape.
type LoadAllMetadataNetworkProfilesListResourceModel struct {
...
func (l *LoadAllMetadataNetworkProfilesListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config LoadAllMetadataNetworkProfilesListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *LoadAllMetadataNetworkProfilesListResource) listRemote(ctx context.Context, config *LoadAllMetadataNetworkProfilesListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["networkProfiles"]
		if !ok {
			diags.AddError("Error listing gigavuecore_load_all_metadata_network_profiles", fmt.Sprintf("Could not decode list page: missing %q array", "networkProfiles"))
			return nil, diags
		}
...
func (l *LoadAllMetadataNetworkProfilesListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
