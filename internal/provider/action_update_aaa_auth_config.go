...
func (r *UpdateAaaAuthConfigAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Update Node Authentication config", Attributes: map[string]schema.Attribute{"auth_sequence": schema.SetAttribute{MarkdownDescription: "Authentication methods order for user login. Valid values are 'local', 'ldap', 'radius', 'tacacs'", Required: true, ElementType: types.StringType}, "cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true}, "external_login_mapping": schema.DynamicAttribute{MarkdownDescription: "Specifies how externally authenticated logins (RADIUS, TACACS+, or LDAP) are mapped to local accounts. Private class", Optional: true}, "non_local_users": schema.DynamicAttribute{MarkdownDescription: "Settings for treating usernames that are not recognized as real accounts (not a locally configured account). Private class", Optional: true}, "password_expiration": schema.DynamicAttribute{MarkdownDescription: "Node Password Expiration config. Private class", Optional: true}, "sshd_max_sessions": schema.Int64Attribute{MarkdownDescription: "Maximum concurrent session that can be logged into devices", Optional: true}, "user_lockout": schema.DynamicAttribute{MarkdownDescription: "Node AAA user lockout settings. Private class", Optional: true}}}
}
