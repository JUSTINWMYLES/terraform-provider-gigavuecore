...
func (r *AdvHashResource) Schema(ctx context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "Return Gigastream Advanced Hash configuration for the given Device Card", Attributes: map[string]schema.Attribute{"cluster_id": schema.StringAttribute{MarkdownDescription: "id of the defining cluster", Required: true}, "fields": schema.SetAttribute{MarkdownDescription: "Required when 'type' == 'fields'. 'gtpteid' requires fields (portsrc and portdst) or (port6src and port6dst)", Optional: true, Computed: true, ElementType: types.StringType}, "slot": schema.StringAttribute{MarkdownDescription: "device card slotId for the gigastream", Required: true}, "slot_id": schema.StringAttribute{Required: true}, "type": schema.StringAttribute{Required: true}}, Blocks: map[string]schema.Block{"timeouts": timeouts.Block(ctx, timeouts.Opts{Create: true, Read: true, Update: true, Delete: true})}}
}
...
func (r *AdvHashResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("slot_id"), req.ID)...)
}
