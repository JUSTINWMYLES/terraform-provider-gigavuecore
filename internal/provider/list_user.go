...
var _ list.ListResource = (*UserListResource)(nil)
var _ list.ListResourceWithConfigure = (*UserListResource)(nil)

// UserListResource is the generated Terraform list resource implementation.
type UserListResource struct {
	client *client.Client
}

// UserListResourceModel describes the gigavuecore_user list filter configuration shape.
...
// NewUserListResource returns a new instance of the generated list resource.
...
func (l *UserListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config UserListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_user", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_user", "List item is missing identity attribute \"username\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_user", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_user", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_user", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *UserListResource) listRemote(ctx context.Context, config *UserListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_user", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_user", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["users"]
		if !ok {
			diags.AddError("Error listing gigavuecore_user", fmt.Sprintf("Could not decode list page: missing %q array", "users"))
			return nil, diags
		}
