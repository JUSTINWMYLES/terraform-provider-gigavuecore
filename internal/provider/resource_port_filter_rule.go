...
type PortFilterRuleResourceModel struct {
	...
	Timeouts  *PortFilterRuleTimeoutsModel `tfsdk:"timeouts"`
}

// PortFilterRuleTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_port_filter_rule resource.
type PortFilterRuleTimeoutsModel struct {
...
func (r *PortFilterRuleResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "Add new filtering rule to a Port", Attributes: map[string]schema.Attribute{"cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true}, "comment": schema.StringAttribute{Optional: true, Computed: true}, "matches": schema.DynamicAttribute{MarkdownDescription: "Set of rule's matching elements. Within a rule, matching elements of the the same type MUST NOT be repeated. The 'position' property of each matching element is not relevant for this rule type as only the outer headers are matched", Required: true}, "port_id": schema.StringAttribute{MarkdownDescription: "id of the target device Port (format: boxId_slotId_port, example: 1_1_c1)", Required: true}, "rule_id": schema.Int64Attribute{MarkdownDescription: "id of the rule to update", Required: true, Validators: []validator.Int64{int64validator.AtLeast(1)}}, "rule_type": schema.StringAttribute{MarkdownDescription: "filter rule type", Required: true}}, Blocks: map[string]schema.Block{"timeouts": schema.SingleNestedBlock{Attributes: map[string]schema.Attribute{"create": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the create operation. Defaults to 1200.", Optional: true}, "read": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the read operation. Defaults to 1200.", Optional: true}, "update": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the update operation. Defaults to 1200.", Optional: true}, "delete": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the delete operation. Defaults to 1200.", Optional: true}}, MarkdownDescription: "Per-operation timeouts in seconds."}}}
}
...
func (r *PortFilterRuleResource) createRemote(ctx context.Context, plan *PortFilterRuleResourceModel, resp *resource.CreateResponse) {
	...
	reqPath = strings.ReplaceAll(reqPath, "{portId}", url.PathEscape(strings.ReplaceAll(plan.PortId.ValueString(), "/", "_")))
...
func (r *PortFilterRuleResource) readRemote(ctx context.Context, state *PortFilterRuleResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	reqPath = strings.ReplaceAll(reqPath, "{portId}", url.PathEscape(strings.ReplaceAll(state.PortId.ValueString(), "/", "_")))
...
func (r *PortFilterRuleResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if len(portFilterRuleImportIDParts) != 3 {
		resp.Diagnostics.AddError("Unexpected Import Identifier", fmt.Sprintf("Expected import identifier with format \"{port_id}:{rule_id}:{cluster_id}\". Got %q.", req.ID))
		return
	}
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("port_id"), portFilterRuleImportIDParts[0])...)
	importRuleId, err := strconv.ParseInt(portFilterRuleImportIDParts[1], 10, 64)
	if err != nil {
		resp.Diagnostics.AddError("Error importing gigavuecore_port_filter_rule", fmt.Sprintf("Could not parse import identifier %q as an integer: %s", portFilterRuleImportIDParts[1], err))
		return
	}
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("rule_id"), importRuleId)...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), portFilterRuleImportIDParts[2])...)
}
