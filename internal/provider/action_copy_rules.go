...
func (r *CopyRulesAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Copy rules from one or more source traffic flow policies to a user-specific clipboard.\nSupports both SOURCE and APPLICATION rule categories with optimistic locking for concurrent conflict detection.", Attributes: map[string]schema.Attribute{"configs": schema.ListNestedAttribute{MarkdownDescription: "List of copy configurations", Required: true, NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{"flow_alias": schema.StringAttribute{MarkdownDescription: "Flow alias (required for APPLICATION category)", Optional: true}, "is_all_rules": schema.BoolAttribute{MarkdownDescription: "Whether to copy all rules or specific rule IDs", Optional: true}, "policy_id_or_alias": schema.StringAttribute{MarkdownDescription: "Policy ID or alias from which to copy rules", Required: true}, "policy_updated_time": schema.Int64Attribute{MarkdownDescription: "Timestamp of the policy version for optimistic locking", Required: true}, "rule_ids": schema.ListAttribute{MarkdownDescription: "Specific rule IDs to copy (if isAllRules is false)", Optional: true, ElementType: types.Int64Type}, "source_and_rule_alias": schema.StringAttribute{MarkdownDescription: "Source and rule alias (required for SOURCE category)", Optional: true}, "sub_flow_alias": schema.StringAttribute{MarkdownDescription: "Sub-flow alias (required for APPLICATION category)", Optional: true}}}}, "rule_category": schema.StringAttribute{MarkdownDescription: "Category of rules to copy/paste", Required: true}, "rule_type": schema.StringAttribute{MarkdownDescription: "Type of application rule", Required: true}}}
}
