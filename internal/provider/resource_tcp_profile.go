...
type TcpProfileResourceModel struct {
	...
	Timeouts       *TcpProfileTimeoutsModel `tfsdk:"timeouts"`
}

// TcpProfileTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_tcp_profile resource.
type TcpProfileTimeoutsModel struct {
...
func (r *TcpProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	tcpProfileImportIDParts := strings.Split(req.ID, ":")
	if len(tcpProfileImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("alias"), tcpProfileImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), tcpProfileImportIDParts[1])...)
}
