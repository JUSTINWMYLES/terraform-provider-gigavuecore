...
var (
	...
	_ resource.ResourceWithIdentity    = (*GsGroupResource)(nil)
...
type GsGroupResourceModel struct {
	...
	Timeouts           *GsGroupTimeoutsModel `tfsdk:"timeouts"`
}

// GsGroupTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_gs_group resource.
type GsGroupTimeoutsModel struct {
...
func (r *GsGroupResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	gsGroupImportIDParts := strings.Split(req.ID, "/")
	if len(gsGroupImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("alias"), gsGroupImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), gsGroupImportIDParts[1])...)
}
