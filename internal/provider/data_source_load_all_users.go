...
func (d *LoadAllUsersDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "Load all Users", Attributes: map[string]schema.Attribute{"items": schema.ListNestedAttribute{Computed: true, NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{"email_id": schema.StringAttribute{MarkdownDescription: "email ID", Computed: true}, "enabled": schema.BoolAttribute{Computed: true}, "full_name": schema.StringAttribute{MarkdownDescription: "user's full name", Computed: true}, "groups": schema.ListAttribute{Computed: true, ElementType: types.StringType}, "password": schema.StringAttribute{MarkdownDescription: "password", Computed: true, Sensitive: true}, "username": schema.StringAttribute{MarkdownDescription: "username", Computed: true}}}}, "page": schema.StringAttribute{MarkdownDescription: "parentheses-enclosed pair of values in a (pageNo:pageSize) format. 'pageNo' is 1-based. If omitted, entire list of entities is returned", Optional: true}, "sort": schema.StringAttribute{MarkdownDescription: "parentheses-enclosed comma-separated list of entity attributes, optionally qualified with the sort order attribute. The default sort order is ASC. Example: sort=(aaa,bbb:ASC,ccc:DESC)", Optional: true}, "username": schema.StringAttribute{MarkdownDescription: "username of the target user", Optional: true}}}
}
...
func (d *LoadAllUsersDataSource) readListRemote(ctx context.Context, config *LoadAllUsersDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_load_all_users", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_users", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["users"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_users", fmt.Sprintf("Could not decode list page: missing %q array", "users"))
			return
		}
