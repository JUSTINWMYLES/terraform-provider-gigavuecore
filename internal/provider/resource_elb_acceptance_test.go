...
func testAccElbResourceConfig(serverURL string) string {
	return fmt.Sprintf("provider \"gigavuecore\" {\n  endpoint = \"%s\"\n  username = \"example\"\n  password = \"example\"\n}\nresource \"gigavuecore_elb\" \"example\" {\n  alias = \"example\"\n  hash_fields = [{\n    hash_field = \"ip\"\n    hash_location = \"inner\"\n  }]\n}\n", serverURL)
}

// newElbResourceMockServer returns an httptest server that stubs the ElbResource CRUD endpoints.
// The server echoes request bodies so that create/update responses reflect the values sent by the test.
func newElbResourceMockServer() *httptest.Server {
	...
	handler0 := func(w http.ResponseWriter, r *http.Request) {
		...
		mu0.Lock()
		defer mu0.Unlock()
		id := strings.Trim(strings.TrimPrefix(r.URL.Path, "/apps/elb"), "/")
		if id == "" {
			id = "example-id"
		}
		switch r.Method {
	...
	mux.HandleFunc("/apps/elb", handler0)
	mux.HandleFunc("/apps/elb/", handler0)
	return httptest.NewServer(mux)
}

// TestAccElbResourceLifecycle verifies create, update, delete, and import flows against a mock API.
func TestAccElbResourceLifecycle(t *testing.T) {
	t.Setenv("TF_ACC", "1")
	server := newElbResourceMockServer()
	defer server.Close()
	resource.Test(t, resource.TestCase{ProtoV6ProviderFactories: map[string]func() (tfprotov6.ProviderServer, error){"gigavuecore": providerserver.NewProtocol6WithError(New())}, Steps: []resource.TestStep{resource.TestStep{Config: testAccElbResourceConfig(server.URL), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("gigavuecore_elb.example", "alias"))}, resource.TestStep{ResourceName: "gigavuecore_elb.example", ImportState: true, ImportStateId: "imported-alias"}}})
}
