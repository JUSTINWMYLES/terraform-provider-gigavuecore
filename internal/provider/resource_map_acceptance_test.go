...
func testAccMapResourceConfig(serverURL string, name string) string {
...
// newMapResourceMockServer returns an httptest server that stubs the MapResource CRUD endpoints.
// The server echoes request bodies so that create/update responses reflect the values sent by the test.
func newMapResourceMockServer() *httptest.Server {
	...
	handler0 := func(w http.ResponseWriter, r *http.Request) {
		...
		mu0.Lock()
		defer mu0.Unlock()
		id := strings.Trim(strings.TrimPrefix(r.URL.Path, "/maps"), "/")
		if id == "" {
			id = "example-id"
		}
		switch r.Method {
	...
	mux.HandleFunc("/maps", handler0)
	mux.HandleFunc("/maps/", handler0)
	return httptest.NewServer(mux)
}

// TestAccMapResourceLifecycle verifies create, update, delete, and import flows against a mock API.
func TestAccMapResourceLifecycle(t *testing.T) {
	t.Setenv("TF_ACC", "1")
	server := newMapResourceMockServer()
	defer server.Close()
	resource.Test(t, resource.TestCase{ProtoV6ProviderFactories: map[string]func() (tfprotov6.ProviderServer, error){"gigavuecore": providerserver.NewProtocol6WithError(New())}, Steps: []resource.TestStep{resource.TestStep{Config: testAccMapResourceConfig(server.URL, "example"), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("gigavuecore_map.example", "alias"), resource.TestCheckResourceAttr("gigavuecore_map.example", "cluster_id", "example"))}, resource.TestStep{Config: testAccMapResourceConfig(server.URL, "updated"), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("gigavuecore_map.example", "alias"), resource.TestCheckResourceAttr("gigavuecore_map.example", "cluster_id", "updated"))}, resource.TestStep{ResourceName: "gigavuecore_map.example", ImportState: true, ImportStateId: "imported-alias/imported-cluster_id"}}})
}
