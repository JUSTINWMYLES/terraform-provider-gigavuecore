...
var (
	...
	_ resource.ResourceWithIdentity    = (*InterfaceResource)(nil)
...
type InterfaceResourceModel struct {
	...
	Timeouts         *InterfaceTimeoutsModel `tfsdk:"timeouts"`
}

// InterfaceTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_interface resource.
type InterfaceTimeoutsModel struct {
...
func (r *InterfaceResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
	resp.IdentitySchema = identityschema.Schema{Attributes: map[string]identityschema.Attribute{"alias": identityschema.StringAttribute{RequiredForImport: true, Description: "ip interface name"}}}
}
...
func (r *InterfaceResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	interfaceImportIDParts := strings.Split(req.ID, "/")
	if len(interfaceImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("alias"), interfaceImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), interfaceImportIDParts[1])...)
}
