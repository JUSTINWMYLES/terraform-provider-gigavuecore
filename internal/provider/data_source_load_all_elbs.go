...
func (d *LoadAllElbsDataSource) readListRemote(ctx context.Context, config *LoadAllElbsDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_load_all_elbs", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_elbs", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["elbs"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_elbs", fmt.Sprintf("Could not decode list page: missing %q array", "elbs"))
			return
		}
