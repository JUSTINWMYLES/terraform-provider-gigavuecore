...
var (
	...
	_ resource.ResourceWithImportState = (*VxlanGroupResource)(nil)
...
type VxlanGroupResourceModel struct {
	...
	Timeouts  *VxlanGroupTimeoutsModel `tfsdk:"timeouts"`
}

// VxlanGroupTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_vxlan_group resource.
type VxlanGroupTimeoutsModel struct {
...
func (r *VxlanGroupResource) readRemote(ctx context.Context, state *VxlanGroupResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	if v, ok := data["circuitTunnelVxlanGroups"]; ok {
		if arr, ok := v.([]any); ok {
			...
			if state.Alias.IsNull() {
				resp.Diagnostics.AddWarning("Error reading gigavuecore_vxlan_group", "The identifier attribute \"alias\" is null in state, so the matching collection element cannot be identified. Falling back to the first element; this may track the wrong instance when the collection has more than one.")
			} else {
...
func (r *VxlanGroupResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	vxlanGroupImportIDParts := strings.Split(req.ID, ":")
	if len(vxlanGroupImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("alias"), vxlanGroupImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), vxlanGroupImportIDParts[1])...)
}
