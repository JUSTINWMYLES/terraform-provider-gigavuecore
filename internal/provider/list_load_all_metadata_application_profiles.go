...
var _ list.ListResourceWithConfigure = (*LoadAllMetadataApplicationProfilesListResource)(nil)
...
// LoadAllMetadataApplicationProfilesListResourceModel describes the gigavuecore_load_all_metadata_application_profiles list filter configuration shape.
type LoadAllMetadataApplicationProfilesListResourceModel struct {
...
func (l *LoadAllMetadataApplicationProfilesListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config LoadAllMetadataApplicationProfilesListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *LoadAllMetadataApplicationProfilesListResource) listRemote(ctx context.Context, config *LoadAllMetadataApplicationProfilesListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["applicationProfiles"]
		if !ok {
			diags.AddError("Error listing gigavuecore_load_all_metadata_application_profiles", fmt.Sprintf("Could not decode list page: missing %q array", "applicationProfiles"))
			return nil, diags
		}
...
func (l *LoadAllMetadataApplicationProfilesListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
