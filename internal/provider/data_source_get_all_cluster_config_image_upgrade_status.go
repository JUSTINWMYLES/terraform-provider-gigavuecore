...
func (d *GetAllClusterConfigImageUpgradeStatusDataSource) readListRemote(ctx context.Context, config *GetAllClusterConfigImageUpgradeStatusDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_get_all_cluster_config_image_upgrade_status", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_all_cluster_config_image_upgrade_status", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["clustersStatus"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_get_all_cluster_config_image_upgrade_status", fmt.Sprintf("Could not decode list page: missing %q array", "clustersStatus"))
			return
		}
