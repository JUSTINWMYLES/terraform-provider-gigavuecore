...
var (
	_ datasource.DataSource              = (*QueryVportTimeSeriesDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*QueryVportTimeSeriesDataSource)(nil)
)

// QueryVportTimeSeriesDataSource is the generated Terraform data source implementation.
type QueryVportTimeSeriesDataSource struct {
	client *client.Client
}

// QueryVportTimeSeriesDataSourceModel describes the data source state shape.
type QueryVportTimeSeriesDataSourceModel struct {
...
func (d *QueryVportTimeSeriesDataSource) readRemote(ctx context.Context, config *QueryVportTimeSeriesDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
		resp.Diagnostics.AddError("Client Not Configured", "The API client was not set on the resource. The provider Configure method must run before resource operations; this is a bug in the generated provider.")
		return
	}
	reqPath := "/trending/vports/timeSeries"
	httpReq, err := d.client.NewRequest(ctx, http.MethodGet, reqPath, nil)
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", fmt.Sprintf("Could not build request: %s", err))
		return
	}
	...
	query.Set("vport", config.Vport.ValueString())
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", fmt.Sprintf("Could not send request: %s", err))
		return
	}
	defer httpResp.Body.Close()
	if httpResp.StatusCode == http.StatusNotFound {
		resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "The requested resource was not found.")
		return
	}
	if !(httpResp.StatusCode == 200) {
		switch httpResp.StatusCode {
		case 400:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Invalid request. See errors payload for details")
			return
		case 401:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Not Authenticated. See errors payload for details")
			return
		case 403:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Access Denied. See errors payload for details")
			return
		case 404:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Entity Not Found. See errors payload for details")
			return
		case 409:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Entity Already Exists. See errors payload for details")
			return
		case 500:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Internal Server Error. See errors payload for details")
			return
		case 503:
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", "Service Unavailable. See errors payload for details")
			return
		default:
			apiErr, err := client.NewAPIError(httpResp)
			if err != nil {
				resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", fmt.Sprintf("Could not read error response: %s", err))
				return
			}
			resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", apiErr.Error())
			return
		}
	}
	...
	if err := decoder.Decode(&data); err != nil && err != io.EOF {
		resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", fmt.Sprintf("Could not decode response body: %s", err))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_query_vport_time_series", fmt.Sprintf("Could not map response to state: %s", err))
		return
	}
}

// Configure stores the API client supplied by the provider.
func (d *QueryVportTimeSeriesDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
