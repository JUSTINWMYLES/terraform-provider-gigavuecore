...
var (
	...
	_ resource.ResourceWithIdentity    = (*LocalUserResource)(nil)
...
type LocalUserResourceModel struct {
	...
	Timeouts        *LocalUserTimeoutsModel `tfsdk:"timeouts"`
}

// LocalUserTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_local_user resource.
type LocalUserTimeoutsModel struct {
...
func (r *LocalUserResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{MarkdownDescription: "Create a new Local User", Attributes: map[string]schema.Attribute{"account_status": schema.StringAttribute{Optional: true, Computed: true, Validators: []validator.String{stringvalidator.OneOf("accountDisabled", "passwordDisabled", "accountLockedOut", "noPassword", "passwordSet")}}, "capability": schema.StringAttribute{Optional: true, Computed: true, Validators: []validator.String{stringvalidator.OneOf("admin", "monitor", "")}}, "cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true}, "current_password": schema.StringAttribute{MarkdownDescription: "Specifies the logged in password", Required: true, Sensitive: true, Validators: []validator.String{stringvalidator.LengthAtLeast(1)}}, "enabled": schema.BoolAttribute{MarkdownDescription: "Temporarily enable/disable logins for the specified account. Disabling an account closes any currently open sessions for the specified account", Optional: true, Computed: true}, "full_name": schema.StringAttribute{MarkdownDescription: "Full name for the account (referred to sometimes as the gecos)", Optional: true, Computed: true}, "roles": schema.ListAttribute{MarkdownDescription: "References to the Roles defined for the User", Optional: true, Computed: true, ElementType: types.StringType}, "user_pwd": schema.StringAttribute{Required: true, Validators: []validator.String{stringvalidator.LengthAtLeast(1)}}, "username": schema.StringAttribute{MarkdownDescription: "Specifies the username of the local user", Required: true}}, Blocks: map[string]schema.Block{"timeouts": schema.SingleNestedBlock{Attributes: map[string]schema.Attribute{"create": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the create operation. Defaults to 1200.", Optional: true}, "read": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the read operation. Defaults to 1200.", Optional: true}, "update": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the update operation. Defaults to 1200.", Optional: true}, "delete": schema.Int64Attribute{MarkdownDescription: "Maximum time in seconds for the delete operation. Defaults to 1200.", Optional: true}}, MarkdownDescription: "Per-operation timeouts in seconds."}}}
}

// IdentitySchema returns the resource identity schema shared with the paired list resource.
func (r *LocalUserResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
	resp.IdentitySchema = identityschema.Schema{Attributes: map[string]identityschema.Attribute{"username": identityschema.StringAttribute{RequiredForImport: true, Description: "Specifies the username of the local user"}}}
}
...
func (r *LocalUserResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	localUserImportIDParts := strings.Split(req.ID, "/")
	if len(localUserImportIDParts) != 2 {
	...
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("username"), localUserImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("cluster_id"), localUserImportIDParts[1])...)
}
