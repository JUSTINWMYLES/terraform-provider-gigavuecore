...
func (e *RegenerateSecretEphemeralResource) Schema(_ context.Context, _ ephemeral.SchemaRequest, resp *ephemeral.SchemaResponse) {
	resp.Schema = ephemeralschema.Schema{MarkdownDescription: "Revoke other user tokens of FM Users. Users with FM Security Management role with write access can access API.", Attributes: map[string]ephemeralschema.Attribute{"context": ephemeralschema.DynamicAttribute{MarkdownDescription: "Gigamon query result context", Optional: true}, "fm_api_token_user_entities": ephemeralschema.DynamicAttribute{Required: true}}}
}
