...
func (d *LoadAllVportsDataSource) readListRemote(ctx context.Context, config *LoadAllVportsDataSourceModel, resp *datasource.ReadResponse) {
	if d.client == nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading gigavuecore_load_all_vports", fmt.Sprintf("Could not read list response: %s", err))
		return
	}
	items := []any{}
	for _, page := range pages {
		...
		if err := dec.Decode(&pageObj); err != nil {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_vports", fmt.Sprintf("Could not decode list page: %s", err))
			return
		}
		pageItems, ok := pageObj["vports"].([]any)
		if !ok {
			resp.Diagnostics.AddError("Error reading gigavuecore_load_all_vports", fmt.Sprintf("Could not decode list page: missing %q array", "vports"))
			return
		}
