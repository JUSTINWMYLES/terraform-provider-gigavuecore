...
var _ list.ListResource = (*GetAllExternalTrapReceiverListResource)(nil)
var _ list.ListResourceWithConfigure = (*GetAllExternalTrapReceiverListResource)(nil)

// GetAllExternalTrapReceiverListResource is the generated Terraform list resource implementation.
type GetAllExternalTrapReceiverListResource struct {
	client *client.Client
}

// GetAllExternalTrapReceiverListResourceModel describes the gigavuecore_get_all_external_trap_receiver list filter configuration shape.
type GetAllExternalTrapReceiverListResourceModel struct {
...
// NewGetAllExternalTrapReceiverListResource returns a new instance of the generated list resource.
func NewGetAllExternalTrapReceiverListResource() list.ListResource {
	return &GetAllExternalTrapReceiverListResource{}
}

// Metadata returns the list resource type name.
func (l *GetAllExternalTrapReceiverListResource) Metadata(_ context.Context, _ resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "gigavuecore_get_all_external_trap_receiver"
}

// ListResourceConfigSchema returns the list resource config schema.
func (l *GetAllExternalTrapReceiverListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
...
func (l *GetAllExternalTrapReceiverListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config GetAllExternalTrapReceiverListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_external_trap_receiver", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *GetAllExternalTrapReceiverListResource) listRemote(ctx context.Context, config *GetAllExternalTrapReceiverListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["externalTrapReceivers"]
		if !ok {
			diags.AddError("Error listing gigavuecore_get_all_external_trap_receiver", fmt.Sprintf("Could not decode list page: missing %q array", "externalTrapReceivers"))
			return nil, diags
		}
...
func (l *GetAllExternalTrapReceiverListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
