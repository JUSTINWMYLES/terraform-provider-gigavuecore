...
func (r *RedefineSyslogConfigAction) Schema(_ context.Context, _ action.SchemaRequest, resp *action.SchemaResponse) {
	resp.Schema = schema.Schema{Description: "Redefine Syslog config", Attributes: map[string]schema.Attribute{"cluster_id": schema.StringAttribute{MarkdownDescription: "Target Cluster ID", Required: true}, "cluster_name": schema.StringAttribute{MarkdownDescription: "Name of the cluster", Required: true}, "syslog_config_list": schema.DynamicAttribute{MarkdownDescription: "List of the syslog configuration specification for every node in the cluster", Required: true}}}
}
