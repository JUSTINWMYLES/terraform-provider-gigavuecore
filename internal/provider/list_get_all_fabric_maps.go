...
var _ list.ListResourceWithConfigure = (*GetAllFabricMapsListResource)(nil)
...
// GetAllFabricMapsListResourceModel describes the gigavuecore_get_all_fabric_maps list filter configuration shape.
type GetAllFabricMapsListResourceModel struct {
...
func (l *GetAllFabricMapsListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		var config GetAllFabricMapsListResourceModel
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if !ok {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_fabric_maps", "List item is missing identity attribute \"alias\".")
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not encode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if err != nil {
				result.Diagnostics.AddError("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not decode list item identity: %s", err))
				if !push(result) {
					return
				}
				continue
			...
			if req.IncludeResource {
				...
				if err != nil {
					result.Diagnostics.AddWarning("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not decode list item into the resource schema: %s", err))
				} else {
...
func (l *GetAllFabricMapsListResource) listRemote(ctx context.Context, config *GetAllFabricMapsListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &pageObj); err != nil {
			diags.AddError("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
		rawItems, ok := pageObj["fabricMaps"]
		if !ok {
			diags.AddError("Error listing gigavuecore_get_all_fabric_maps", fmt.Sprintf("Could not decode list page: missing %q array", "fabricMaps"))
			return nil, diags
		}
...
func (l *GetAllFabricMapsListResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
