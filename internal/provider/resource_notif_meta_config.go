...
var (
	...
	_ resource.ResourceWithImportState = (*NotifMetaConfigResource)(nil)
...
type NotifMetaConfigResourceModel struct {
	...
	Timeouts              *NotifMetaConfigTimeoutsModel `tfsdk:"timeouts"`
}

// NotifMetaConfigTimeoutsModel describes the per-operation timeout configuration (in seconds) for the gigavuecore_notif_meta_config resource.
type NotifMetaConfigTimeoutsModel struct {
...
func (r *NotifMetaConfigResource) createRemote(ctx context.Context, plan *NotifMetaConfigResourceModel, resp *resource.CreateResponse) {
	...
	reqPath = strings.ReplaceAll(reqPath, "{notifType}", url.PathEscape(plan.Type.ValueString()))
	reqPath = strings.ReplaceAll(reqPath, "{taskId}", url.PathEscape(plan.TaskId.ValueString()))
	...
	if plan.TaskId.IsNull() || plan.TaskId.IsUnknown() {
		...
		if loc != "" {
			...
			plan.TaskId = types.StringValue(loc)
		} else {
...
func (r *NotifMetaConfigResource) readRemote(ctx context.Context, state *NotifMetaConfigResourceModel, resp *resource.ReadResponse) (removed bool) {
	...
	reqPath := "/notification/event/notifMetaConfig/{notifType}/{taskId}"
	reqPath = strings.ReplaceAll(reqPath, "{notifType}", url.PathEscape(state.Type.ValueString()))
	reqPath = strings.ReplaceAll(reqPath, "{taskId}", url.PathEscape(state.TaskId.ValueString()))
...
func (r *NotifMetaConfigResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	defer cancel()
	if plan.TaskId.IsNull() || plan.TaskId.IsUnknown() {
		if !state.TaskId.IsNull() && !state.TaskId.IsUnknown() {
...
func (r *NotifMetaConfigResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	notifMetaConfigImportIDParts := strings.Split(req.ID, "/")
	if len(notifMetaConfigImportIDParts) != 2 {
		resp.Diagnostics.AddError("Unexpected Import Identifier", fmt.Sprintf("Expected import identifier with format \"{type}/{task_id}\". Got %q.", req.ID))
		return
	}
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("type"), notifMetaConfigImportIDParts[0])...)
	resp.Diagnostics.Append(resp.State.SetAttribute(ctx, path.Root("task_id"), notifMetaConfigImportIDParts[1])...)
}
