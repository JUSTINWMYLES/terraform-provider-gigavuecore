...
* `only_traffic` (Bool, required) - If true, restrict to traffic only configurations
* `protocol` (String, required) - File Source/File Destination protocol. http and https only applicable for fileSource
* `remote` (Bool, required) - If true, upload the text configuration file to remote host.If false, save file to local storage
