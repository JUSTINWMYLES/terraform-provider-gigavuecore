...
~> **Warning:** This action accepts attributes whose names indicate secrets (secretKey, secretKey), but action schemas cannot mark attributes Sensitive, so their values are stored and displayed in plain text. Avoid passing real secrets through these attributes.
