---
page_title: "gigavuecore_query_fabric_map_time_series Data Source - gigavuecore"
subcategory: ""
description: |-
  Fabric Map Time Series Query interface
---

# gigavuecore_query_fabric_map_time_series Data Source
...
```terraform
data "gigavuecore_query_fabric_map_time_series" "example" {
  end_time   = "example"
  map        = "example"
  max_points = 0
  max_series = 0
...
* `map` (String, optional) - Map alias to filter by. Value of '\*' indicates a 'group-by-map' request. Multiple values separated by '\|' can be specified, in which case time series are filtered and grouped by the requested map
