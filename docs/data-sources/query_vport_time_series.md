---
page_title: "gigavuecore_query_vport_time_series Data Source - gigavuecore"
subcategory: ""
description: |-
  vPort Time Series Query interface
---

# gigavuecore_query_vport_time_series Data Source
...
```terraform
data "gigavuecore_query_vport_time_series" "example" {
  cluster     = "example"
  end_time    = "example"
  gs_group    = "example"
  max_points  = 0
  max_series  = 0
  metric      = "gsVport.octets"
  rate        = true
  since       = "example"
  sort_by     = "example"
  start_time  = "example"
...
* `vport` (String, required) - vPort alias to filter by. Value of '\*' indicates a 'group-by-vPort' request. Multiple values separated by '\|' can be specified, in which case time series are filtered and grouped by the requested vPort
