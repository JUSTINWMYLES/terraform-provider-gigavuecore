...
* `binding_errors` (Boolean) - uused for floating licenses
* `bindings` (Attributes List) - unused for VBL licenses (see [below for nested schema](#nestedatt--items--bindings))
* `bundle` (String) - the bundle family (bundles of a single family can be active at any point of time on an FM)
...
* `feature` (String) - comma-separated list of features that this license comprises of
...
* `num_licenses` (Number) - total volume allowance is this number times the allowance set by the SKU definition
...
* `total_volume` (String) - in Gigabytes per day
