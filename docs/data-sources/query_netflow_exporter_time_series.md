---
page_title: "gigavuecore_query_netflow_exporter_time_series Data Source - gigavuecore"
subcategory: ""
description: |-
  Netflow Exporter Time Series Query interface
---

# gigavuecore_query_netflow_exporter_time_series Data Source
...
```terraform
data "gigavuecore_query_netflow_exporter_time_series" "example" {
  cluster     = "example"
  end_time    = "example"
  gs_group    = "example"
  max_points  = 0
  max_series  = 0
  metric      = "nfExporter.recordAdded"
  nf_exporter = "example"
  since       = "example"
  sort_by     = "example"
  start_time  = "example"
...
* `nf_exporter` (String, optional) - Netflow Exporter alias to filter by. Value of '\*' indicates a 'group-by-NfExporter' request. Multiple values separated by '\|' can be specified, in which case time series are filtered and grouped by the requested NfExporter
