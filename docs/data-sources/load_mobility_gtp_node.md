...
* `additional_tool_ports` (List of String, computed) - Additional Tool ports for the GTP node which takes a copy of traffic along with vport that is added into the first level maps. Its a list of ports of format cluster:port
...
* `deployed` (Boolean, computed) - True when the GTP node is attempted for deployment
...
* `node_override_network_ports` (List of String, computed) - Network ports for the GTP node. Its a list of ports of format cluster:port
...
* `traffic_sources` (Attributes List, computed) - List of all traffic sources for the GTP node (see [below for nested schema](#nestedatt--traffic_sources))
