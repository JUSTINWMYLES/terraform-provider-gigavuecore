---
page_title: "gigavuecore_query_tunneled_port_time_series Data Source - gigavuecore"
subcategory: ""
description: |-
  Tunnel Port Time Series Query interface
---

# gigavuecore_query_tunneled_port_time_series Data Source
...
```terraform
data "gigavuecore_query_tunneled_port_time_series" "example" {
  box_port    = "example"
  cluster     = "example"
  end_time    = "example"
  gs_group    = "example"
...
* `box_port` (String, optional) - BoxPort id to filter by. This is a {boxId} part of the full cluster port id {boxId}/{slotId}/{portId}. Value of '\*' indicates a 'group-by-port' request. Multiple values separated by '\|' can be specified, in which case time series are filtered and grouped by the requested ports
