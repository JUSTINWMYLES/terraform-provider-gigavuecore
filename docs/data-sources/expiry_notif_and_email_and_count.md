...
* `items` (List(Number), computed) - first three numbers are for floating licenses, second three are for node-locked licenses; the numbers signify count expiring in 90 days, expiring soon (within notification period), and expired (beyond end date)
