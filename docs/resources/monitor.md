...
* `records` (Set of String, computed) - Aliases of referenced Netflow Records defined on the GsGroup. Up to 5 records.
* `sampling` (Attributes, computed) - monitor sampling (see [below for nested schema](#nestedatt--sampling))
* `sampling_space` (Number, computed) - DEPRECATED: use 'sampling'
* `ssl_port_restrictions` (Attributes, computed) - Port restrictions for Netflow/SSL sessions (see [below for nested schema](#nestedatt--ssl_port_restrictions))
