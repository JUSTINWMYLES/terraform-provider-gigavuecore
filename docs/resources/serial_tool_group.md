...
* `enabled` (Bool, optional) - setting to false is equivalent to forcing the inline serial tool failure (useful for taking the inline serial tool out of commission for maintenance or other purposes)
...
* `inline_tools` (List(String), required) - a list of aliases of the inline tools or inline tool groups that participate in the series
...
* `enabled` (Bool, computed) - setting to false is equivalent to forcing the inline serial tool failure (useful for taking the inline serial tool out of commission for maintenance or other purposes)
